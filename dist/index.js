...
class TranslationService {
  constructor(publicKey) {
    ...
    __publicField(this, "cacheMetrics", { hits: 0, misses: 0 });
...
const getUserLanguage = () => {
  ...
  const userLanguage = languages.find(
    (lang) => userLanguages.includes(lang.code)
  );
...
const _TranslationWidget = class _TranslationWidget {
  ...
  setupContentObserver() {
    this.observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        ...
        if (mutation.type === "characterData" || mutation.type === "childList" && Array.from(mutation.addedNodes).some(
  ...
  createWidget() {
    ...
    this.elements = {
      trigger: this.widget.querySelector(
        ".jigts-widget-trigger"
      ),
      dropdown: this.widget.querySelector(
        ".jigts-widget-dropdown"
      ),
      searchInput: this.widget.querySelector(
        ".jigts-search-input"
      ),
      clearSearch: this.widget.querySelector(
        ".jigts-clear-search"
      ),
      languageItems: this.widget.querySelectorAll(
        ".jigts-language-item"
      ),
      loadingIndicator: this.widget.querySelector(
  ...
  getCurrentLanguageLabel() {
    var _a;
    return ((_a = languages.find(
      (language) => language.code === this.currentLanguage
    )) == null ? void 0 : _a.native) || "English";
  }
  ...
  updateLoadingState(isLoading) {
    var _a, _b;
    const triggerContent = (_a = this.elements.trigger) == null ? void 0 : _a.querySelector(
      ".jigts-trigger-content"
    );
    const triggerLoading = (_b = this.elements.trigger) == null ? void 0 : _b.querySelector(
  ...
  resetToDefaultLanguage() {
    ...
    this.currentLanguage = this.config.pageLanguage;
    const languageItems = this.widget.querySelectorAll(
      ".jigts-language-item"
    );
  ...
  async _translatePage(targetLang) {
    ...
    try {
      ...
      const allTranslatedTexts = await Promise.all(
        nonEmptyBatchTexts.map(
          (texts) => this.translationService.translateBatchText(texts, targetLang)
        )
      );
      ...
      nodes.forEach((node, nodeIdx) => {
        const parent = node.parentElement;
        const batchIdx = nonEmptyBatchNodes.findIndex(
          (batch) => batch.includes(node)
        );
        if (batchIdx !== -1) {
  ...
  updateResetButtonVisibility() {
    const resetButton = this.widget.querySelector(
      ".jigts-reset-option"
    );
    if (resetButton) {
  ...
  setupEventListeners() {
    ...
    if (resetButton) {
      resetButton.addEventListener("click", () => {
        ...
        this.isTranslated = false;
        this.updateResetButtonVisibility();
        const languageItems2 = this.widget.querySelectorAll(
          ".jigts-language-item"
        );
    ...
    searchInput.addEventListener("input", () => {
      ...
      const items = this.widget.querySelectorAll(
        ".jigts-language-item"
      );
    ...
    languageItems.forEach((item) => {
      item.addEventListener("click", async () => {
        ...
        const langName = (_a = item.querySelector(
          ".jigts-language-name"
        ...
        const triggerIcon = (_b = this.elements.trigger) == null ? void 0 : _b.querySelector(
          ".jigts-trigger-icon"
        );
        ...
        if (langCode && langCode !== this.currentLanguage) {
          ...
          const triggerLoading = trigger.querySelector(
            ".jigts-trigger-loading"
          );
          if (triggerContent && triggerLoading) {
            triggerContent.style.display = "none";
...
window.translate = async (langCode, onComplete) => {
